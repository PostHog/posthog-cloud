...
class TestMonthlyBillingRecord(BaseTest, PlanTestMixin):
    def test_can_only_create_one_billing_record_per_month_per_organization(self):
        organization, _, _ = self.create_org_team_user()
        organization_billing = OrganizationBilling.objects.create(
            organization=organization
        )

        record = MonthlyBillingRecord.objects.create(
            organization_billing=organization_billing,
            billing_period=datetime.date(2020, 1, 1),
        )
        record.refresh_from_db()
        count: int = MonthlyBillingRecord.objects.count()

        with self.assertRaises(ValidationError) as e:
            MonthlyBillingRecord.objects.create(
                organization_billing=organization_billing,
                billing_period=datetime.date(2020, 1, 1),
            )
        ...
        self.assertEqual(
            ...
            {
                "__all__": [
                    "Monthly billing record with this Organization billing and Billing period already exists."
                ]
            },
        )
        self.assertEqual(MonthlyBillingRecord.objects.count(), count)

    def test_cannot_report_less_than_zero_events(self):
        organization, _, _ = self.create_org_team_user()
        organization_billing = OrganizationBilling.objects.create(
            organization=organization
        )
        ...
        self.assertEqual(
            e.exception.message_dict,
            {"event_usage": ["Ensure this value is greater than or equal to 0."]},
        )
        self.assertEqual(MonthlyBillingRecord.objects.count(), count)

    def test_cannot_create_billing_record_without_a_period(self):
        organization, _, _ = self.create_org_team_user()
        organization_billing = OrganizationBilling.objects.create(
            organization=organization
        )
        ...
        self.assertEqual(
            e.exception.message_dict, {"billing_period": ["This field cannot be null."]}
        )
        self.assertEqual(MonthlyBillingRecord.objects.count(), count)
